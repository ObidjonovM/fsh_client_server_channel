from client_server_channel.models import SupplierTable
...
class SupplierC:
    ...
    def add(supp_info):
        now = datetime.now()
        supp_info['date_added'] = now,
        supp_info['date_modified'] = now,
        add_result = SupplierTable.insert(supp_info)
    ...
    @staticmethod
    def get(supplier_id):
        get_result = SupplierTable.get(supplier_id)
    ...
    @staticmethod
    def get_all():
        get_all_result = SupplierTable.get_all()
    ...
    @staticmethod
    def get_ids_names():
        ids_names = SupplierTable.get_ids_names()
    ...
    def get_names_by_ids(suppliers_ids):
        names_ids = SupplierTable.get_names_by_ids(suppliers_ids)
    ...
    def update(supp_info):
        get_result = SupplierTable.get(supp_info['supplier_id'])
        log_code = utls.record_log(get_result, 'update', 'crud_logs')
        if get_result['data'] != []:
            supp_info['date_modified'] = datetime.now()
            update_result = SupplierTable.update(supp_info)
            return {
    ...
    def delete(supplier_id):
        ...
        if get_result['data'] != []:
            delete_result = SupplierTable.delete(supplier_id)
            return {
