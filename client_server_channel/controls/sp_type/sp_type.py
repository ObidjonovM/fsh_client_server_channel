from client_server_channel.models import SpTypeTable
...
class SpTypeC:
    ...
    @staticmethod
    def add(sp_type_info):
        now = datetime.now()
        sp_type_info['date_added'] = now,
        sp_type_info['date_modified'] = now,
        add_result = SpTypeTable.insert(sp_type_info)
    ...
    @staticmethod
    def get(sp_type_id):
        get_result = SpTypeTable.get(sp_type_id)
    ...
    @staticmethod
    def get_all():
        get_all_result = SpTypeTable.get_all()
    ...
    @staticmethod
    def get_ids_names():
        ids_names = SpTypeTable.get_ids_names()
    ...
    def get_names_by_ids(sp_types_ids):
        names_ids = SpTypeTable.get_names_by_ids(sp_types_ids)
    ...
    def update(sp_type_info):
        get_result = SpTypeTable.get(sp_type_info['sp_type_id'])
        log_code = utls.record_log(get_result, 'update', 'crud_logs')
        if get_result['data'] != []:
            sp_type_info['date_modified'] = datetime.now()
            update_result = SpTypeTable.update(sp_type_info)
            return {
    ...
    def delete(sp_type_id):
        ...
        if get_result['data'] != []:
            delete_result = SpTypeTable.delete(sp_type_id)
            return {
